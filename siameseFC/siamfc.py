...
class TrackerSiamFC(Tracker):
    ...
    def track_video(self,videoPath):
        ...
        out = cv2.VideoWriter(videoPath+'output.avi',fourcc, 20.0, (1080,720))
        ret, frame = cap.read()
        img_traing = np.zeros((np.shape(frame)[0],np.shape(frame)[1],3),dtype=np.uint8)
        while(cap.isOpened()):
            if np.shape(frame) == ():
                break
                ...
                while(cap.isOpened()):
                    ...
                    if not np.shape(frame) == () :
                        ...
                        if n == 0:
                            ...
                            pro = 1.00
                            box = cv2.selectROI('roi', inputImage, False, False )
                        ...
                        n +=1
                        outimage,box_local= ops.show_image(inputImage,img_traing,box_out,pro=pro)
                        out.write(outimage)
                        if (box_local[0] < 5) or (box_local[1] < 5) or ((1280- box_local[0])) < ((box_local[2])) or ((720- box_local[1]) < (box_local[3])) or (pro < 1.00):
                            box_local = [100,100,100,100]
                            box = cv2.selectROI('roi', cv2.resize(frame,(1080,720)), False, False )
                            pro = 0.00
                            self.init(inputImage, box)
                            cv2.destroyAllWindows()
                    ret, frame = cap.read()    
